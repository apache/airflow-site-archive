...
* versioned sub-projects (have a ``stable/`` directory) -> both ``stable/`` and the
  numbered version it mirrors (e.g. ``3.2.2``) are touched, since ``stable`` is a
  byte-for-byte copy of that release and both paths are served;
...
# A release version directory name, e.g. "2.10.5" or "3.0.0" (optionally suffixed).
VERSION_RE = re.compile(r"^[0-9]+\.[0-9]+")


def _version_key(name: str):
    """Natural sort key so 3.2.10 > 3.2.2 (plain string sort would get this wrong)."""
    return [(1, int(p)) if p.isdigit() else (0, p) for p in re.split(r"[.\-+]", name)]


def stable_version_dir(sub: Path, stable: Path) -> Path | None:
    """Return the numbered version directory that ``stable`` mirrors.

    ``stable`` is a byte-for-byte copy of the latest release, so we pick the version
    directory whose ``index.html`` matches ``stable/index.html``; if none matches (or
    there is no index), fall back to the highest version by natural sort.
    """
    candidates = [
        d for d in sub.iterdir() if d.is_dir() and d.name != "stable" and VERSION_RE.match(d.name)
    ]
    if not candidates:
        return None
    ref_index = stable / "index.html"
    if ref_index.is_file():
        ref = ref_index.read_bytes()
        matching = [
            d for d in candidates if (d / "index.html").is_file() and (d / "index.html").read_bytes() == ref
        ]
        if matching:
            return max(matching, key=lambda d: _version_key(d.name))
    return max(candidates, key=lambda d: _version_key(d.name))


def iter_target_dirs(docs_path: Path):
    """Yield ``(label, directory)`` to process for the latest version of each sub-project.

    For versioned sub-projects both ``stable/`` and the numbered version it mirrors
    (e.g. ``3.2.2``) are emitted; for non-versioned ones the whole sub-project is.
    """
    for sub in sorted(p for p in docs_path.iterdir() if p.is_dir()):
        stable = sub / "stable"
        if stable.is_dir():
            yield sub.name, stable  # versioned -> latest == stable
            versioned = stable_version_dir(sub, stable)
            if versioned is not None:
                yield f"{sub.name} ({versioned.name})", versioned  # the version stable mirrors
        else:
